...
.. note::
  ...
  Unit tests for **SQLAthanor** are written using `pytest`_ and
...
**SQLAthanor**, we decided to focus on practicality. That means:
  ...
  * **Coverage matters...kind of.** We have documented the primary intended
    behavior of every function in the **SQLAthanor** library, and the
...
**SQLAthanor** does not use any custom command-line options in its
test suite.
...
.. note::
  ...
  Because of the simplicity of **SQLAthanor**, the test suite does
...
.. note::
  ...
  The **SQLAthanor** test suite does support incremental testing using,
